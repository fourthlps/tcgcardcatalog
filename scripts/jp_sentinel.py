...
States: ok | warn | skipped | no_results | stale
  - no_results  relay fetch produced nothing usable today          -> FAIL
  - stale       newest JP entry is 2+ days old (~36h at the 20:30Z
                schedule, given date-only price timestamps)        -> FAIL
  - warn        coverage below proposed threshold — REPORT-ONLY until
                the threshold is founder-approved                  -> pass
  - skipped     dry-run / staged JP_SETS / JP_ALLOW_SKIP dev runs  -> pass
"""
...
COVERAGE_RATIO     = 0.85   # PROPOSED (warn-only): refreshed >= 85% of last good run
COVERAGE_FLOOR     = 2000   # PROPOSED (warn-only): absolute minimum refreshed count
COVERAGE_DEFAULT_BASELINE = 2306   # 2026-07-18 run, used until a baseline is recorded

FAILING_STATES = ("no_results", "stale")
...
def compute():
    ...
    baseline = prev.get("refreshed_last_run") or 0
    if baseline <= 0:
        baseline = COVERAGE_DEFAULT_BASELINE
    coverage_min = max(COVERAGE_FLOOR, int(baseline * COVERAGE_RATIO))
    ...
    if dry_run or staged or allow_skip:
        ...
        state = "warn"   # report-only until the coverage threshold is approved
    else:
        state = "ok"
    ...
    print(f"[jp_sentinel] state={state} last_refresh={block['last_refresh']} "
          ...
          f"coverage {'OK' if coverage_ok else 'BELOW'} (>= {coverage_min}, report-only)")
    return state
